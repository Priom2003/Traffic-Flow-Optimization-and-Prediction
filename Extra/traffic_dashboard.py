...
#     # Load feature CSV (without target labels)
#     df = pd.read_csv("phase3_structured_dataset.csv")  # or another CSV for live input

#     # # --- Prepare Features ---
#     # embedding_cols = [f"emb_{i}" for i in range(384)]
#     # structured_cols = ['sentiment_numeric', 'sentiment_score',
#     #                    'latitude', 'longitude', 'hour', 'day', 'weekday', 'month']
...
#     # df["RF_Prediction"] = rf_model.predict(X_scaled)
#     # df["XGB_Prediction"] = xgb_model.predict(X_scaled)
#     # lstm_pred_probs = lstm_model.predict(X_lstm)
#     # df["LSTM_Prediction"] = np.argmax(lstm_pred_probs, axis=1)
    
#     # --- Prepare Features ---
#     embedding_cols = [f"emb_{i}" for i in range(384)]
#     structured_cols = ['sentiment_numeric', 'sentiment_score','cluster_label', 'latitude', 'longitude', 'hour', 'day', 'weekday', 'month']
#     train_cols = embedding_cols + structured_cols
...
#     # --- Standardize using a saved scaler if available ---
#     # If you have saved scaler: scaler = joblib.load("scaler.pkl")
#     # Else, just use a new one (may reduce accuracy)
#     scaler = StandardScaler()
#     X_scaled = scaler.fit_transform(X)
